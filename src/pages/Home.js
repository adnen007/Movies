...
const Home = () => {
  ...
  if (popular.length === 0 || commingSoon.length === 0 || top.length === 0 || playingNow === 0) {
    return <Loading />;
  }
  return (
    <div className="home">
      <div className="trending">
        ...
        <div className="slider">
          <div className="left" onClick={() => onRowClick(window.innerWidth >= 768 ? 46 : 92)}>
            <BsChevronDoubleLeft />
          </div>
          <div className="right" onClick={() => onRowClick(window.innerWidth >= 768 ? -46 : -92)}>
            <BsChevronDoubleRight />
          </div>

          <div style={{ transform: `translateX(calc( ${x}vw ) )` }}>
            {playingNow.map((e) => {
              return (
